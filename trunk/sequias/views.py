...
#*********************************** REPORTE DESCARGABLE EN WORD *******************************
    
def __hoja_words__(request):
    fecha1=request.session['fecha_inicio']
    fecha2=request.session['fecha_final']
    if request.session['comunidad']:
        com = request.session['comunidad'].id
    ...
    casos = encuestas.count()
    #TODO: Sumatorias de maiz,frijol,sorgo CICLO PRIMERA
    # 2=Maiz, 3=Frijol, 4=sorgo
    sorgo_planea = encuestas.filter(primera__producto__id=4).aggregate(Sum('primera__planea_siembra'))['primera__planea_siembra__sum']
    arroz_sembrada = encuestas.filter(primera__producto__id=4).aggregate(Sum('primera__area_sembrada'))['primera__area_sembrada__sum']
    #----------
    frijol_planea = encuestas.filter(primera__producto__id=3).aggregate(Sum('primera__planea_siembra'))['primera__planea_siembra__sum']
    frijol_sembrada = encuestas.filter(primera__producto__id=3).aggregate(Sum('primera__area_sembrada'))['primera__area_sembrada__sum']
    #------------
    maiz_planea = encuestas.filter(primera__producto__id=2).aggregate(Sum('primera__planea_siembra'))['primera__planea_siembra__sum']
    maiz_sembrada = encuestas.filter(primera__producto__id=2).aggregate(Sum('primera__area_sembrada'))['primera__area_sembrada__sum']
    #TODO: area cosechada
    arroz_cosechada = encuestas.filter(primera__producto__id=4).aggregate(Sum('primera__area_cosechada'))['primera__area_cosechada__sum']
    frijol_cosechada = encuestas.filter(primera__producto__id=3).aggregate(Sum('primera__area_cosechada'))['primera__area_cosechada__sum']
    maiz_cosechada = encuestas.filter(primera__producto__id=2).aggregate(Sum('primera__area_cosechada'))['primera__area_cosechada__sum']
    #TODO:area perdida
    try:
        arroz_perdida = (arroz_sembrada - arroz_cosechada)
    except:
    ...
    try:
        ...
        pass
    #TODO: produccion
    arroz_produccion = encuestas.filter(primera__producto__id=4).aggregate(Sum('primera__produccion'))['primera__produccion__sum']
    frijol_produccion = encuestas.filter(primera__producto__id=3).aggregate(Sum('primera__produccion'))['primera__produccion__sum']
    maiz_produccion = encuestas.filter(primera__producto__id=2).aggregate(Sum('primera__produccion'))['primera__produccion__sum']
    #TODO: rendimientos
    try:
        arroz_rendi = arroz_produccion / arroz_cosechada
    except:
    ...
    #TODO: CICLO POSTRERA, arroz=sorgo :) :P
    sorgo_planea_p = encuestas.filter(postrera__producto__id=4).aggregate(Sum('postrera__planea_siembra'))['postrera__planea_siembra__sum']
    arroz_sembrada_P = encuestas.filter(postrera__producto__id=4).aggregate(Sum('postrera__area_sembrada'))['postrera__area_sembrada__sum']
    #---------
    frijol_planea_p = encuestas.filter(postrera__producto__id=3).aggregate(Sum('postrera__planea_siembra'))['postrera__planea_siembra__sum']
    frijol_sembrada_P = encuestas.filter(postrera__producto__id=3).aggregate(Sum('postrera__area_sembrada'))['postrera__area_sembrada__sum']
    #-----------
    maiz_planea_p = encuestas.filter(postrera__producto__id=2).aggregate(Sum('postrera__planea_siembra'))['postrera__planea_siembra__sum']
    maiz_sembrada_P = encuestas.filter(postrera__producto__id=2).aggregate(Sum('postrera__area_sembrada'))['postrera__area_sembrada__sum'] 
    #TODO: area cosechada
    arroz_cosechada_P = encuestas.filter(postrera__producto__id=4).aggregate(Sum('postrera__area_cosechada'))['postrera__area_cosechada__sum']
    frijol_cosechada_P = encuestas.filter(postrera__producto__id=3).aggregate(Sum('postrera__area_cosechada'))['postrera__area_cosechada__sum']
    maiz_cosechada_P = encuestas.filter(postrera__producto__id=2).aggregate(Sum('postrera__area_cosechada'))['postrera__area_cosechada__sum']
    #TODO:area perdida
    try:
        arroz_perdida_P = arroz_sembrada_P - arroz_cosechada_P
    except:
    ...
    try:
        ...
        pass
    #TODO: produccion
    arroz_produccion_P = encuestas.filter(postrera__producto__id=4).aggregate(Sum('postrera__produccion'))['postrera__produccion__sum']
    frijol_produccion_P = encuestas.filter(postrera__producto__id=3).aggregate(Sum('postrera__produccion'))['postrera__produccion__sum']
    maiz_produccion_P = encuestas.filter(postrera__producto__id=2).aggregate(Sum('postrera__produccion'))['postrera__produccion__sum']
    #TODO: rendimientos
    try:
        arroz_rendi_P = arroz_produccion_P / arroz_cosechada_P
    except:
    ...
    #Ciclo Apante
    sorgo_planea_a = encuestas.filter(apante__producto__id=4).aggregate(Sum('apante__planea_siembra'))['apante__planea_siembra__sum']
    arroz_sembrada_A = encuestas.filter(apante__producto__id=4).aggregate(Sum('apante__area_sembrada'))['apante__area_sembrada__sum']
    #------------
    frijol_planea_a = encuestas.filter(apante__producto__id=3).aggregate(Sum('apante__planea_siembra'))['apante__planea_siembra__sum']
    frijol_sembrada_A = encuestas.filter(apante__producto__id=3).aggregate(Sum('apante__area_sembrada'))['apante__area_sembrada__sum']
    #-------------
    maiz_planea_a = encuestas.filter(apante__producto__id=2).aggregate(Sum('apante__planea_siembra'))['apante__planea_siembra__sum']
    maiz_sembrada_A = encuestas.filter(apante__producto__id=2).aggregate(Sum('apante__area_sembrada'))['apante__area_sembrada__sum']
    #TODO: area cosechada
    arroz_cosechada_A = encuestas.filter(apante__producto__id=4).aggregate(Sum('apante__area_cosechada'))['apante__area_cosechada__sum']
    frijol_cosechada_A = encuestas.filter(apante__producto__id=3).aggregate(Sum('apante__area_cosechada'))['apante__area_cosechada__sum']
    maiz_cosechada_A = encuestas.filter(apante__producto__id=2).aggregate(Sum('apante__area_cosechada'))['apante__area_cosechada__sum']
    #TODO:area perdida
    try:
        arroz_perdida_A = (arroz_sembrada_A - arroz_cosechada_A)
    except:
    ...
    try:
        ...
        pass
    #TODO: produccion
    arroz_produccion_A = encuestas.filter(apante__producto__id=4).aggregate(Sum('apante__produccion'))['apante__produccion__sum']
    frijol_produccion_A = encuestas.filter(apante__producto__id=3).aggregate(Sum('apante__produccion'))['apante__produccion__sum']
    maiz_produccion_A = encuestas.filter(apante__producto__id=2).aggregate(Sum('apante__produccion'))['apante__produccion__sum']
    #TODO: rendimientos
    try:
        arroz_rendi_A = arroz_produccion_A / arroz_cosechada_A
    except:
...
#---------------- graficos de primera, postrera y apante -------------------
    gperdida = encuestas
    casos = gperdida.count()
    maiz_s = 0
    maiz_c = 0
...
#    for encuesta in gperdida:
    ...
    mensa3_A = "Grafico razones de perdida"
    #Envios de las url solo para primera
    url_o = grafos.make_graph(lista, legends, mensa, return_json=False)
    url1_o = grafos.make_graph(lista1, legends1, mensa1, return_json=False)
    url2 = grafos.make_graph(lista2, legends2, mensa2, return_json=False)
    url6 = grafos.make_graph(lista3, legends3, mensa3, return_json=False)
    #Envios de las url solo para postrera
    url3 = grafos.make_graph(lista_P, legends_P, mensa_P, return_json=False)
...
#---------------- fin de los graficos de primera, postrera y apante --------
...
#------------- grafico de la disponibilidad ----------------------
    ...
    url_disp_formula = grafos.make_graph(data, legends, message, multiline=True, 
                           return_json=False, type=GroupedVerticalBarChart)
#---------------- fin de los grafos de disponibilidad ---------------------------
#------------- comienzo de la nutricion --------------------------
    nutri = encuestas
    casos = nutri.count()
    # niños de 1-5 años
    ninos_normal = nutri.filter(nutricion__edad__range=(1,5), nutricion__ninos__contains="ninos",  nutricion__brazalete__id=1).count()
...
#---------------- grafico de la nutricion -----------------
    ...
    url = grafos.make_graph(lista1,legends1,mensa1,return_json=False)
    url1 = grafos.make_graph(lista2,legends2,mensa2,return_json=False)
#----------------- fin del grafico de nutricion ------------

    dict = locals()
    
    return dict

#*********************************** comienza hoja de calculo y word *******************************************
@session_required
...
@session_required    
def hoja_de_word(request):
    dict = __hoja_words__(request)
    return write_word('encuesta/hoja_reporte.html', dict, 'hoja_de_reporte.doc')
...
def write_word(template_src, context_dict, filename):
    response = render_to_response(template_src, context_dict)
    response['Content-Disposition'] = 'attachment; filename='+filename
